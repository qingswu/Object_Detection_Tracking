...
def pack2(config):
    ...
    # this way it will only allocate nessasary
    tfconfig.gpu_options.allow_growth = True
    with tf.Session(config=tfconfig) as sess:
        initialize(load=True, load_best=config.load_best,
...
class Mask_RCNN_FPN_frozen():
    def __init__(self, modelpath, gpuid):
        ...
        # input place holders
        self.image = self.graph.get_tensor_by_name(
            "%s/image:0" % self.var_prefix)
...
class Mask_RCNN_FPN():
    def __init__(self, config, gpuid=0):
        ...
        ], dtype='int32', initializer=tf.constant_initializer(0), trainable=False)
        ...
        for k in xrange(len(config.anchor_strides)):
            self.anchor_labels.append(tf.placeholder(
                tf.int32, [None, None, num_anchors], name="anchor_labels_lvl%s" % (k+2)))
            self.anchor_boxes.append(tf.placeholder(
                tf.float32, [None, None, num_anchors, 4], name="anchor_boxes_lvl%s" % (k+2)))
        ...
        for i in xrange(len(config.small_objects)):
            self.so_gt_boxes.append(tf.placeholder(
                tf.float32, [None, 4], name="so_gt_boxes_c%s" % (i+1)))
            self.so_gt_labels.append(tf.placeholder(
                tf.int64, [None, ], name="so_gt_labels_c%s" % (i+1)))
    ...
    def get_all_anchors_fpn(self):
        ...
        for stride, size in zip(config.anchor_strides, config.anchor_sizes):
            anchors_np = get_all_anchors(stride=stride, sizes=[
                                         size], ratios=config.anchor_ratios, max_size=config.max_size)
        ...
        for i, stride in enumerate(config.anchor_strides):
            with tf.name_scope("FPN_slice_lvl%s" % (i)):
                if i < 3:
                    ...
                    p23456[i] = tf.slice(pi, [0, 0, 0, 0], tf.concat(
                        [[-1, -1], target_shape], axis=0))
                ...
                self.sliced_anchor_boxes.append(
                    tf.slice(gt_anchor_boxes[i], [0, 0, 0, 0], slice4d))
        ...
        for lvl in xrange(num_lvl):
            with tf.name_scope("Lvl%s" % (lvl+2)):
                ...
                pred_boxes_decoded = decode_bbox_target(
                    multilevel_box_logits[lvl], anchors, decode_clip=config.bbox_decode_clip)

                this_fpn_nms_topk = fpn_nms_topk
                proposal_boxes, proposal_scores = generate_rpn_proposals(tf.reshape(pred_boxes_decoded, [-1, 4]), tf.reshape(
                    multilevel_label_logits[lvl], [-1]), image_shape2d, config, pre_nms_topk=this_fpn_nms_topk)
    ...
    def build_forward(self):
        ...
        c2345 = resnet_fpn_backbone(image, config.resnet_num_block, use_gn=config.use_gn, resolution_requirement=config.fpn_resolution_requirement, use_dilations=config.use_dilations,
                                    use_deformable=config.use_deformable, tf_pad_reverse=True, freeze=config.freeze, use_basic_block=config.use_basic_block, use_se=config.use_se, use_resnext=config.use_resnext)

        # include lateral 1x1 conv and final 3x3 conv
        # -> [7, 7, 256]
        p23456 = fpn_model(
            c2345, num_channel=config.fpn_num_channel, use_gn=config.use_gn, scope="fpn")
        ...
        self.slice_feature_and_anchors(
            image_shape2d, p23456, multilevel_anchors)
        ...
        rpn_outputs = [self.rpn_head(pi, config.fpn_num_channel, len(
            config.anchor_ratios), data_format="NCHW", scope="rpn") for pi in p23456]
        multilevel_label_logits = [k[0] for k in rpn_outputs]
        multilevel_box_logits = [k[1] for k in rpn_outputs]

        if config.freeze_rpn:
            multilevel_label_logits = [tf.stop_gradient(
                o) for o in multilevel_label_logits]
            multilevel_box_logits = [tf.stop_gradient(
                o) for o in multilevel_box_logits]
        ...
        proposal_boxes, proposal_scores = self.generate_fpn_proposals(
            multilevel_anchors, multilevel_label_logits, multilevel_box_logits, image_shape2d)
        ...
        roi_feature_fastrcnn = self.multilevel_roi_align(
            p23456[:4], rcnn_boxes, 7)

        if config.use_frcnn_class_agnostic:
            # (N,num_class), (N, 1, 4)
            fastrcnn_label_logits, fastrcnn_box_logits = self.fastrcnn_2fc_head_class_agnostic(
                roi_feature_fastrcnn, config.num_class, boxes=rcnn_boxes, scope="fastrcnn")
        else:
        ...
        if config.use_small_object_head:
            ...
            boxes = decode_bbox_target(fastrcnn_box_logits / tf.constant(
                config.fastrcnn_bbox_reg_weights, dtype=tf.float32), anchors)
            ...
            small_object_class_ids = [
                config.classname2id[name] - 1 for name in config.small_objects]

            # C is the number of small object class
            # [C, N, 4], [C, N]
            so_boxes, so_scores = tf.gather(boxes, small_object_class_ids), tf.gather(
                probs, small_object_class_ids)

            # 1. we do NMS for each class to get topk

            # for each catagory get the top K
            # [C, K, 4] / [C, K]
            so_boxes, so_scores = tf.map_fn(self.nms_return_boxes, (so_scores, so_boxes), dtype=(
            ...
            with tf.variable_scope("fastrcnn", reuse=tf.AUTO_REUSE):
                ...
                # [C, K, dim]
                hidden = tf.reshape(
                    hidden, [len(config.small_objects), -1, dim])
                ...
                if config.use_so_association:
                    ...
                    ref_selection = tf.image.non_max_suppression(
                        ref_boxes, ref_scores, max_output_size=ref_topk, iou_threshold=config.fastrcnn_nms_iou_thres)
                    # [Rr, 4]
                    ref_boxes = tf.gather(ref_boxes, ref_selection)
                    ref_scores = tf.gather(ref_scores, ref_selection)
                    ref_feat = self.multilevel_roi_align(
                        p23456[:4], ref_boxes, 7)

                    # share the same fc
                    ref_feat = dense(
                        ref_feat, dim, W_init=initializer, activation=tf.nn.relu, scope="fc6")
                    ref_feat = dense(
                        ref_feat, dim, W_init=initializer, activation=tf.nn.relu, scope="fc7")
                    ...
                    for i in xrange(len(config.small_objects)):
                        if config.use_so_association:
                            asso_hidden = hidden[i] + person_object_relation(
                                hidden[i], self.so_boxes[i], ref_boxes, ref_feat, group=16, geo_feat_dim=64, scope="person_object_relation")
                            so_label_logits.append(dense(asso_hidden, 2, W_init=tf.random_normal_initializer(
                                stddev=0.01), scope="small_object_classification_c%s" % (i+1)))
                        else:
                            so_label_logits.append(dense(hidden[i], 2, W_init=tf.random_normal_initializer(
                                stddev=0.01), scope="small_object_classification_c%s" % (i+1)))
                    add_wd(0.0001)
        ...
        if config.is_train:
            ...
            fg_fastrcnn_box_logits = tf.gather(
                fastrcnn_box_logits, fg_inds_wrt_sample)
            ...
            encoded_boxes = encode_bbox_target(
                matched_gt_boxes, fg_sampled_boxes) * tf.constant(config.fastrcnn_bbox_reg_weights)  # [10,10,5,5]?
            ...
            fastrcnn_label_loss, fastrcnn_box_loss = self.fastrcnn_losses(
                rcnn_labels, fastrcnn_label_logits, encoded_boxes, fg_fastrcnn_box_logits)
            ...
            if config.use_small_object_head:
                ...
                # so_labels [C, K] # [0, 1]
                so_labels = get_so_labels(
                    self.so_boxes, self.so_gt_boxes, self.so_gt_labels, config=config)

                so_label_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
                    labels=so_labels, logits=so_label_logits)

                so_label_loss = tf.reduce_mean(
                    so_label_loss, name="label_loss")
            ...
            # mask rcnn loss
            if config.add_mask:
                fg_inds_wrt_sample = tf.reshape(
                    tf.where(rcnn_labels > 0), [-1])
                ...
                mrcnn_loss = self.maskrcnn_loss(
                    mask_logits, fg_labels, target_masks_for_fg)
            ...
            if config.use_frcnn_class_agnostic:
                ...
                fastrcnn_box_logits = tf.tile(
                    fastrcnn_box_logits, [1, config.num_class-1, 1])
            ...
            decoded_boxes = decode_bbox_target(fastrcnn_box_logits / tf.constant(
                config.fastrcnn_bbox_reg_weights, dtype=tf.float32), anchors)
            ...
            if config.use_small_object_head:
                ...
                new_label_logits = []
                # BG is ignore anyway
                new_label_logits.append(tf.reduce_mean(
                    so_label_logits[:, :, 0], axis=0))  # [K]
                for i in xrange(len(config.small_objects)):
                ...
                so_pred_indices, so_final_probs = self.fastrcnn_predictions(
                    so_decoded_boxes, so_label_logits, no_score_filter=not config.use_so_score_thres)
            ...
            if config.use_cpu_nms:
                ...
                assert boxes.shape[1] == config.num_class - \
                    1, (boxes.shape, config.num_class)
            ...
            if config.add_mask:
                ...
                indices = tf.stack(
                    [tf.range(tf.size(final_labels)), tf.to_int32(final_labels)-1], axis=1)
    ...
    def rpn_head(self, featuremap, channel, num_anchors, data_format, scope="rpn"):
        with tf.variable_scope(scope):
            ...
            label_logits = tf.squeeze(
                tf.transpose(label_logits, [0, 2, 3, 1]), 0)

            box_shape = tf.shape(box_logits)
            box_logits = tf.transpose(
                box_logits, [0, 2, 3, 1])  # [1,FS1, FS2,1024*4]
            # [FS1, FS2,1024,4]
    ...
    def small_object_classification_head(self, feature, num_class, scope="small_object_classification"):
        ...
        with tf.variable_scope(scope):
            hidden = dense(feature, dim, W_init=initializer,
                           activation=tf.nn.relu, scope="fc6")
            hidden = dense(hidden, dim, W_init=initializer,
                           activation=tf.nn.relu, scope="fc7")

            classification = dense(hidden, num_class, W_init=tf.random_normal_initializer(
                stddev=0.01), scope="class")  # [K,num_class]
        ...
        with tf.variable_scope(scope):
            ...
            if config.use_conv_frcnn_head:
                hidden = self.conv_frcnn_head(
                    feature, dim, config.conv_frcnn_head_dim, num_conv=4, use_gn=config.use_gn)
            else:
                ...
                if config.add_relation_nn:
                    hidden = dense(feature, dim, W_init=initializer,
                                   activation=tf.nn.relu, scope="fc6")
                    hidden = hidden + \
                        relation_network(hidden, boxes, group=16,
                                         geo_feat_dim=64, scope="RM_r1")
                    hidden = dense(hidden, dim, W_init=initializer,
                                   activation=tf.nn.relu, scope="fc7")
                    hidden = hidden + \
                        relation_network(hidden, boxes, group=16,
                                         geo_feat_dim=64, scope="RM_r2")
                else:
                    hidden = dense(feature, dim, W_init=initializer,
                                   activation=tf.nn.relu, scope="fc6")
                    hidden = dense(hidden, dim, W_init=initializer,
            ...
            if config.use_att_frcnn_head:
                ...
                # [K, 7, 7, 1]
                attention = conv2d(feature, 1, kernel=3, padding="SAME", stride=1, activation=tf.nn.softmax,
                                   use_bias=True, data_format="NHWC", W_init=initializer, scope="attention")
                # [K,7*7, C]
                ...
                # match the dimension
                attended_feat = dense(
                    attended, dim, W_init=initializer, activation=tf.nn.relu, scope="att_trans")
            ...
            with tf.variable_scope("outputs"):
                ...
                box_regression = dense(
                    hidden, num_class*4, W_init=tf.random_normal_initializer(stddev=0.001), scope="box")
    ...
    def conv_frcnn_head(self, feature, fc_dim, conv_dim, num_conv, use_gn=False):
        ...
        for k in xrange(num_conv):
            l = conv2d(l, conv_dim, kernel=3, activation=tf.nn.relu, data_format="NCHW", W_init=tf.variance_scaling_initializer(
                scale=2.0, mode="fan_out", distribution='truncated_normal'), scope="conv%s" % (k))
            if use_gn:
                l = group_norm(l, scope="gn%s" % (k))
        l = dense(l, fc_dim, W_init=tf.variance_scaling_initializer(),
    ...
    def maskrcnn_up4conv_head(self, feature, num_class, scope="maskrcnn_head"):
        ...
        with tf.variable_scope(scope):
            for k in xrange(num_conv):
                l = conv2d(l, config.mrcnn_head_dim, kernel=3, activation=tf.nn.relu, data_format="NCHW", W_init=tf.variance_scaling_initializer(
                    scale=2.0, mode="fan_out", distribution='truncated_normal'), scope="fcn%s" % (k))

            l = deconv2d(l, config.mrcnn_head_dim, kernel=2, stride=2, activation=tf.nn.relu, data_format="NCHW",
                         W_init=tf.variance_scaling_initializer(scale=2.0, mode="fan_out", distribution='truncated_normal'), scope="deconv")
    ...
    def nms_return_masks(self, X):
        config = self.config
        prob, box, select_threshold = X  # [K], [K,4]
        output_shape = tf.shape(prob)
        # [K]
        ids = tf.reshape(tf.where(prob >= select_threshold), [-1])
        prob_ = tf.gather(prob, ids)
        box_ = tf.gather(box, ids)
        # NMS
        selection = tf.image.non_max_suppression(
            box_, prob_, max_output_size=config.result_per_im, iou_threshold=config.fastrcnn_nms_iou_thres)
        selection = tf.to_int32(tf.gather(ids, selection))
        sorted_selection = -tf.nn.top_k(-selection, k=tf.size(selection))[0]

        mask = tf.sparse_to_dense(sparse_indices=sorted_selection,
                                  output_shape=output_shape, sparse_values=True, default_value=False)
    ...
    def nms_return_masks_no_score_filter(self, X):
        ...
        # NMS
        selection = tf.image.non_max_suppression(
            box, prob, max_output_size=config.result_per_im, iou_threshold=config.fastrcnn_nms_iou_thres)
        sorted_selection = -tf.nn.top_k(-selection, k=tf.size(selection))[0]
    ...
    def fastrcnn_predictions(self, boxes, probs, no_score_filter=False, scope="fastrcnn_predictions"):
        with tf.variable_scope(scope):
            ...
            if no_score_filter:
                ...
                masks = tf.map_fn(
                    self.nms_return_masks, (probs, boxes, self.select_threshold), dtype=tf.bool, parallel_iterations=10)
            ...
            # topk_indices [R]
            topk_probs, topk_indices = tf.nn.top_k(probs, tf.minimum(
                config.result_per_im, tf.size(probs)), sorted=False)
        ...
        with tf.variable_scope(scope):
            for lvl in xrange(num_lvl):
                ...
                label_loss, box_loss = self.rpn_losses(
                    gt_labels, encoded_gt_boxes, multilevel_label_logits[lvl], multilevel_box_logits[lvl], scope="level%s" % (lvl+2))
    ...
    def rpn_losses(self, anchor_labels, anchor_boxes, label_logits, box_logits, scope="rpn_losses"):
        ...
        with tf.variable_scope(scope):
            ...
            valid_mask = tf.stop_gradient(
                tf.not_equal(anchor_labels, -1))  # 1,0|pos/neg
            pos_mask = tf.stop_gradient(tf.equal(anchor_labels, 1))
            nr_valid = tf.stop_gradient(tf.count_nonzero(
                valid_mask, dtype=tf.int32), name="num_valid_anchor")
            nr_pos = tf.identity(tf.count_nonzero(
                pos_mask, dtype=tf.int32), name='num_pos_anchor')
            ...
            if config.focal_loss:
                ...
                label_loss = tf.reduce_sum(
                    label_loss) * (1. / config.rpn_batch_per_im)

            label_loss = tf.where(tf.equal(nr_valid, 0),
                                  placeholder, label_loss, name='label_loss')
            ...
            box_loss = tf.losses.huber_loss(
                pos_anchor_boxes, pos_box_logits, delta=delta, reduction=tf.losses.Reduction.SUM) / delta
            ...
            box_loss = tf.where(tf.equal(nr_pos, 0),
                                placeholder, box_loss, name='box_loss')
        ...
        with tf.variable_scope(scope):
            ...
            if config.focal_loss:
                onehot_label = tf.one_hot(
                    labels, label_logits.get_shape()[-1])  # [N, num_classes]
            ...
            # /  N_FG + N_NEG ?
            box_loss = tf.truediv(box_loss, tf.to_float(
                tf.shape(labels)[0]), name='box_loss')
    ...
    def get_feed_dict(self, batch, is_train=False):
            ...
            if config.use_mixup:
                ...
                if use_mixup:
                    ...
                    batch.data['gt'][0]['boxes'] = np.concatenate(
                        [batch.data['gt'][0]['boxes'], batch.data['gt'][0]['mixup_boxes']], axis=0)
        ...
        if batch.data.has_key("resized_image"):
            ...
            resized_image = resizeImage(
                image, short_edge_size, config.max_size)
        ...
        if is_train:
            ...
            if config.flip_image:
                ...
                rand = random.random()
                if rand > prob:
                    resized_image = cv2.flip(
                        resized_image, 1)  # 1 for horizontal
                    # boxes[:,0,0] = neww - boxes[:,0,0] - boxes[:,0,1] # for (x,y,w,h)
                    boxes[:, 0] = neww - boxes[:, 0]
                    # (x_min will be x_max after flip)
                    boxes[:, 0, :] = boxes[:, 0, ::-1]
            ...
            if config.vis_pre:
                ...
                o_boxes_x1x2 = np.asarray(
                    [box_wh_to_x1x2(box) for box in o_boxes])
                boxes_x1x2 = np.asarray([box for box in boxes])
                ori_vis = draw_boxes(image, o_boxes_x1x2, labels=label_names)
                new_vis = draw_boxes(
                    resized_image, boxes_x1x2, labels=label_names)
                imgname = os.path.splitext(
                    os.path.basename(batch.data['imgs'][0]))[0]
                cv2.imwrite("%s.ori.jpg" % os.path.join(
                    config.vis_path, imgname), ori_vis)
                cv2.imwrite("%s.prepro.jpg" % os.path.join(
                    config.vis_path, imgname), new_vis)
            ...
            assert len(multilevel_anchor_labels) == len(multilevel_anchor_boxes) == len(self.anchor_labels) == len(
                self.anchor_boxes), (len(multilevel_anchor_labels), len(multilevel_anchor_boxes), len(self.anchor_labels), len(self.anchor_boxes))
            ...
            if config.use_small_object_head:
                for si in xrange(len(config.small_objects)):
                    ...
                    # the box ids
                    so_ids = [i for i in xrange(
                        len(labels)) if labels[i] == small_object_class_id]
                    # small object label id is different
                    # so_label is 0/1, so should be all 1s
                    feed_dict[self.so_gt_boxes[si]
                              ] = boxes[so_ids, :]  # could be empty
                    feed_dict[self.so_gt_labels[si]] = [
                        1 for i in xrange(len(so_ids))]
        else:
    ...
    def get_rpn_anchor_input(self, im, boxes):
        ...
        all_anchors_np = np.copy(get_all_anchors(
            stride=config.anchor_stride, sizes=config.anchor_sizes, ratios=config.anchor_ratios, max_size=config.max_size))
        ...
        anchor_labels, anchor_boxes = self.get_anchor_labels(
        ...
        featuremap_labels = - \
            np.ones((featureh * featurew*config.num_anchors,), dtype='int32')
    ...
    def get_multilevel_rpn_anchor_input(self, im, boxes):
        ...
        assert end == num_all_anchors, ("num all anchors:%s, end:%s" % (
            num_all_anchors, end))
    ...
    def get_anchor_labels(self, anchors, gt_boxes):
        ...
        def filter_box_label(labels, val, max_num):
            ...
            if len(cur_inds) > max_num:
                disable_inds = np.random.choice(cur_inds, size=(
                    len(cur_inds) - max_num), replace=False)
...
class RCNN_FPN_givenbox():
    ...
    def build_forward(self):
        ...
        c2345 = resnet_fpn_backbone(image, config.resnet_num_block, use_gn=config.use_gn, resolution_requirement=config.fpn_resolution_requirement, use_dilations=config.use_dilations,
                                    use_deformable=config.use_deformable, tf_pad_reverse=True, freeze=config.freeze, use_basic_block=config.use_basic_block, use_se=config.use_se)
        ...
        # [N, 256]
        self.final_box_features = tf.reduce_mean(
            roi_feature_fastrcnn, axis=[2, 3])
...
def initialize(load, load_best, config, sess):
    ...
    if load:
        ...
        allvars = [var for var in allvars if var.name.split(
            ":")[0].split("/")[-1] not in opts]
        # so allvars is actually the variables except things for training

        if config.ignore_gn_vars:
            allvars = [
                var for var in allvars if "/gn" not in var.name.split(":")[0]]
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            if os.path.exists(load_from):
                if load_from.endswith(".ckpt"):
                    ...
                    params = {get_op_tensor_name(
                        n)[1]: v for n, v in dict(weights).iteritems()}
                    ...
                    if len(not_used) > 0:
                        print("warning, %s/%s in npz not restored:%s" %
                              (len(weights.keys()) - len(intersect), len(weights.keys()), not_used))
